...
【生成のルール】
- main_content: 画像から読み取った情報とユーザーのメモを組み合わせて、読者を惹きつける本文を作成。150-300文字程度。絵文字を効果的に使用。見出しは【】で囲む。適度に改行を入れて読みやすくすること。
- key_points: 投稿の核となるポイントを箇条書き（✅や✨で装飾）で3-5個。【重要】各ポイントは必ず改行で区切ること（カンマ区切りにしない）。
- call_to_action: 読者に次のアクションを促す一言。「詳細はプロフィールのリンクから」など。

【フォーマットの重要ルール】
- 各セクション（main_content、key_points、call_to_action）の間は空行で区切ること
- key_pointsの各項目は必ず1行ずつ改行して出力すること（カンマで繋げない）
- main_contentも2-3文ごとに改行を入れて読みやすくすること
