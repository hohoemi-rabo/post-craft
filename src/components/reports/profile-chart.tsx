...
import type { ProfileBreakdown } from '@/types/reports'
  ...
  if (data.length === 0) {
    return (
      <div className="p-6 bg-white/5 border border-white/10 rounded-xl">
        <h3 className="text-sm font-bold text-white mb-4">プロフィール別</h3>
    ...
    <div className="p-6 bg-white/5 border border-white/10 rounded-xl">
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="w-full space-y-2">
          {data.map((item, i) => (
            <div key={item.profileId || 'none'} className="flex items-center gap-2 text-sm">
              <span
                className="w-3 h-3 rounded-full flex-shrink-0"
                style={{ backgroundColor: CHART_COLORS[i % CHART_COLORS.length] }}
              />
              <span className="text-slate-300">
                {item.profileIcon} {item.profileName}
              </span>
