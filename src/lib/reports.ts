...
export async function getReportData(userId: string, period: PeriodFilter): Promise<ReportData> {
  ...
  for (const post of allPosts) {
    typeCounts.set(post.post_type_id, (typeCounts.get(post.post_type_id) || 0) + 1)
  }

  const postTypeBreakdown: TypeBreakdown[] = Array.from(typeCounts.entries())
    .map(([typeId, count]) => {
  ...
  for (const post of allPosts) {
    profileCounts.set(post.profile_id, (profileCounts.get(post.profile_id) || 0) + 1)
  }

  const profileBreakdown: ProfileBreakdown[] = Array.from(profileCounts.entries())
    .map(([profId, count]) => {
  ...
  for (const post of allPosts) {
    ...
    if (existing) {
      ...
      weekMap.set(key, { start: ws, end: we, count: 1 })
    }
  }
