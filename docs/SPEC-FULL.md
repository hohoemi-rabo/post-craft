...
- `image_read`: 画像読み取り専用の5ステップフロー（画像+メモ→キャッチコピー→生成→完成）。複数プロフィールで利用可能（例: `image_read`, `image_read_biz`）
...
requireRemakeSuggestionOwnership(id, userId); // → { error, suggestion }
```
...
1. タイプ選択 → 2. 画像アップロード + メモ → 3. キャッチコピー確認 → 4. 生成中 → 5. 完成
...
- 対象: 全投稿タイプ（`image_read` フローを含む）
- 参照数: 1投稿のみ
- キャプション: 冒頭に前回の投稿を1文で触れる導入文を自動生成
- ハッシュタグ: 前回のハッシュタグを優先的に再利用 + 追加生成（計10個）
- 画像設定: 3択（前回設定を引き継ぐ / 新しく設定する / 画像なし）※標準フローのみ
- DB: `posts.related_post_id` で関連を記録
- UI: `StepContentInput` / `StepImageReadInput` のトグル式セレクタ
...
- 確認画面でユーザーが編集・再生成可能（30文字以内）（標準フロー・image_readフロー共通）
...
- `/reports` - Server Component + Suspense（Recharts は `next/dynamic` + `ssr: false` で遅延読み込み）
- メニュー: 📊 投稿レポート
...
```
page.tsx (Server Component)
  ├── ヘッダー + PeriodFilter (即表示)
  └── <Suspense fallback={<ReportsSkeleton />}>
       └── ReportsContent (Server async: getReportData())
            └── ReportsContentClient (Client: dynamic import でチャート描画)
```
...
| `useRemakeSuggestions` | `useRemakeSuggestions.ts` | リメイク提案の取得・生成・削除・使用済み     |
...
| `remake.ts`         | `RemakeSuggestion`, `toRemakeSuggestion()`                                                                                       |
| `reports.ts`        | `ReportData`, `PeriodFilter`, `TypeBreakdown`, `ProfileBreakdown`                                                                |
