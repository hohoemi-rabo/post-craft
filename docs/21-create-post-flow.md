...
### 1. 投稿作成ページ構造
- [ ] `/app/(dashboard)/create/page.tsx` - メインページ（ステップ管理）
- [ ] ステップ状態管理（useState or URL params）
- [ ] 進行状況インジケーター

### 2. Step 1: 投稿タイプ選択
- [ ] `components/create/step-post-type.tsx` 作成
- [ ] 4タイプのカード表示
  ```
  🔧 解決タイプ    📢 宣伝タイプ
  💡 Tipsタイプ    ✨ 実績タイプ
...
### 3. Step 2: 内容入力
- [ ] `components/create/step-content-input.tsx` 作成
- [ ] メモ書き入力エリア
  - プレースホルダーにタイプ別の例文
  - 文字数カウント
- [ ] URL 入力オプション（記事抽出）
...
### 4. Step 3: 画像設定
- [ ] `components/create/step-image-settings.tsx` 作成
- [ ] 画像スタイル選択
  ```
  ○ マンガ風（男性）  ○ マンガ風（女性）
  ○ ピクセルアート    ○ イラスト（人物なし）
...
### 5. Step 4: 生成実行
- [ ] `components/create/step-generating.tsx` 作成
- [ ] 生成中のローディング表示
- [ ] 進捗ステータス
  ```
  ✅ 文章を生成中... → 完了
...
### 6. Step 5: 結果確認
- [ ] `components/create/step-result.tsx` 作成
- [ ] または `/app/(dashboard)/create/result/page.tsx`
- [ ] 表示項目
  - 生成された投稿文（編集可能）
  - ハッシュタグ（選択・追加可能）
  - 生成された画像
