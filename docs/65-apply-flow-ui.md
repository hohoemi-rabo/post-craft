...
- [x] 確認ダイアログコンポーネントを実装
- [x] `handleApply()` 関数を実装（`useState` でペンディング管理、Button の `loading` prop 活用）
- [x] 適用 API の呼び出しロジックを実装
- [x] 成功状態の表示コンポーネントを実装
- [x] 遷移リンク（投稿作成、プロフィール設定、分析一覧）を実装
- [x] トースト通知（成功・エラー）を実装
- [x] 既に適用済みの場合の表示を実装（`existingConfig.status === 'applied'` + `generated_profile_id` で初期表示）
- [x] ペンディング状態中のボタン無効化を実装（`loading` prop + ダイアログ外クリック無効化）
- [x] レスポンシブデザインを確認（`flex-col sm:flex-row`、ダイアログ `max-w-md mx-4`）
